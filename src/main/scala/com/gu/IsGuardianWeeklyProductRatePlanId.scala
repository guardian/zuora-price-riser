...
/**
  * Predicate asserting that given Product Pate Plan ID is a
  * Guardian Weekly Product Rate Plan ID
...
*/
object IsGuardianWeeklyProductRatePlanId extends (RatePlan => Boolean) {
  def apply(ratePlan: RatePlan): Boolean = ids.contains(ratePlan.productRatePlanId)
